 ...
 * @作者 DHCC-SONG
 * @日期 Jun 16, 2016
 * @描述	[8号消息队列（与支付系统发送通讯报文）处理器]
 */
public class SendMsgServerGrant implements Runnable {
	    Logger logger = Logger.getLogger(SendMsgServerGrant.class);
	    
		public void run() {
			Jedis jedis = RedisUtil.getThreadJedis();
			while(true){
				try {
					List<String> list = jedis.brpop(RedisUtil.BRPOP_TIMEOUT, RedisUtil.QUEUE8);
					String message = null;
					if( list != null ){
						message = list.get(1);
						if(message != null){
							WorkSendMsg msg = new WorkSendMsg(message,PUBConstant.BACK_TYPE_01);
							ThreadPoolManager.getInstance8().exec(msg); //  加入线程池进行执行
						}
					} else {
						logger.info("没有读到消息，redis链接将休息"+RedisUtil.WHILE_WAIT+"毫秒后重新阻塞读取消息,管道名称:"+RedisUtil.QUEUE8);
						Thread.sleep(RedisUtil.WHILE_WAIT);
					}
				}catch (JedisConnectionException e){
					logger.info("Redis链接异常，请检查服务器网络，系统正在重连...:"+RedisUtil.QUEUE8);
					try {
						Thread.sleep(RedisUtil.WHILE_WAIT);
