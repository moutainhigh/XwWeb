 ...
 $.extend(Calendar.prototype,{
	...
	_renderCalendarPanel: function(){
		...
		if(options.type == 'yyyy-mm-dd HH:MM' || options.type == 'yyyy-mm-dd HH:MM:SS') {
			var	date_time = $('<div class="date_time"><span class="spinner"><input type="text" class="spinner-text" id="timeUquine" onblur="checkTime()"><i class="spinner-arrow spinner-arrow-up"></i><i class="spinner-arrow spinner-arrow-down"></i></span></div>')
			.appendTo(date_pane);
		}
		var	date_button = $('<div class="date_button"><a href="javascript:void(0)" class="date-current">����</a><a href="javascript:void(0)" class="date-closed">�ر�</a><a href="javascript:void(0)" class="date-ok" onclick="checkTime()">ȷ��</a></div>').appendTo(date_pane);
...
function checkTime(){
	 ...
	 var timeTwo = document.getElementById("timeUquine").value;
	 var arr = new Array();
		arr = timeTwo.split(":");
		if (arr[0] < 0 || arr[0] >= 24) {
			alert("��ʼʱ�䣺СʱӦ����00-24֮�䡣��");	
			return false;
		}
		if (arr[1] < 0 || arr[1] >= 60) {		
			alert("��ʼʱ�䣺����Ӧ����00-60֮�䡣��");	
			return false;
		}
	return true;
 }