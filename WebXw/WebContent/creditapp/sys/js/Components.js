...
function ComSaveTime(combo, typeid, xmlDoc) {
	...
	for (i = 0; i < (xmlroot.childNodes.length); i++){//对每一个结点进行处理
		if (xmlroot.childNodes.item(i).childNodes.item(4).text == typeid) {
			...
			combo.options.add(oOption);
			oOption.innerText = GetHowLong(xmlroot.childNodes.item(i).childNodes
					.item(3).text);
...
//获取基金属性
function ComFundType(combo, xmlDoc) {
	var xmlroot = xmlDoc.documentElement;
	for (i = 0; i < (xmlroot.childNodes.length); i++){//对每一个结点进行处理
		var oOption = document.createElement("OPTION");
		combo.options.add(oOption);
...
function NBround(nb, len) {
	...
	if (nb < 0.0000001)
		return new Number(0); // 小于0.1E-7的按0处理
	pos = strnb.indexOf(".");
	if ((pos == -1) || (strnb.length - pos - 1) < len)
		...
		if (nblast < 5)
			...
			if (strnb.substring(0, 1) == "-")
				return new Number(new String(new Number(strnb.substring(0, pos
						+ len + 1))
						- Math.pow(10, -1 * len)).substring(0, pos + len + 1));
			else
				return new Number(new String(new Number(strnb.substring(0, pos
						+ len + 1))
						+ Math.pow(10, -1 * len)).substring(0, pos + len + 1));
...
function GetDayLen(Date1, Date2) {
	...
	// 以每月30天算一月，一年为360天
	var DiffDay = (StartDate.getFullYear() - StandDate.getFullYear()) * 360
			+ (StartDate.getMonth() - StandDate.getMonth()) * 30
			+ (StartDate.getDate() - StandDate.getDate());
