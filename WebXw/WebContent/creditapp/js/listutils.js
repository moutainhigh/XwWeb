 ...
 * ��̬����hidden��
 * @param form ��
 * @param elementName Ԫ������ hidden name=""
 * @param elementVu  Ԫ��ֵ hidden value
 */

function dynamicHiddenElement(form, elementName, elementVu) {

	var objNewNode = document.createElement("INPUT");
	objNewNode.type = "hidden";
	objNewNode.name = elementName;
	objNewNode.value = elementVu;
	form.appendChild(objNewNode);
}
/**
 * ɾ��form��������
 * @param form
 * @returns
 */
function clearFormElement(form) {
 ...
 * 
 */
function isSignleSelected(returnStrId){
	if(returnStrId!=""){
		var ids=returnStrId.split(",");
   		if(ids.length==2){
   			return true;
   		}
   	}
 ...
 * 
 */
function isMoreSelected(returnStrId){
   	var ids=returnStrId.split(",");
   	if(ids.length>1){
   		return true;
   	}
