...
TreePanel = function(config){
	...
	this.iconPath = 'creditapp/js/tree/'+treeIconPath+"/";	
	this.clickListeners = [];
...
TreeNode=function(attributes) {
	...
	this.checked = (this.checked==null ||this.checked=='null') ? false : this.checked;
...
TreeNode.prototype={
	...
	paintClipIcoImg : function(flag){
		if(this.isRoot()){
			this['html-element']['clip'].style.display='none';
			return;
		}
		var ownerTree = this.getOwnerTree();
		var icon = 'empty';
	...
	collapse : function(deep){
		var flag = this.isExpand;
		this.isExpand=false;
		if(flag != false){
		this['html-element']['child'].style.display='none';
		}
		this.paintIconImg(flag);
		this.paintClipIcoImg(flag);
		if(deep){
	...
	modChild : function(newnode){
		this['attributes']['text']=newnode['attributes']['text'];
		this['attributes']['id']=newnode['attributes']['id'];
		this['attributes']['url']=newnode['attributes']['url'];
		this['attributes']['lvl']=newnode['attributes']['lvl'];
		if(this['html-element']){
    		this.paintPrefix();
    	}
		return newnode;
