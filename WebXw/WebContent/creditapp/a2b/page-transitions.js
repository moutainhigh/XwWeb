(function() {
	...
	var is_anim = false;
	var current = 0;
	var lastUi=$("#lastUi").val();
	var currFlag = "A";
	var changeFlag = (lastUi == null || lastUi == "") ? "A":lastUi;
...
//		var next_page = $('#' + $(link).attr(data_prefix + 'to'));
	...
	function link_clicked() {
		all_links.on('click', function(e) {
			if(currFlag =="A"){
				currFlag="B";
				$("#navDiv").css("z-index","8");
				$("#navDiv").hide();
			}
			else {
				currFlag="A";
				$("#navDiv").css("z-index","10");
				$("#navDiv").show("2000");
			}
			e.preventDefault();
			pages(this);
	...
	function init() {
		set_current_page();
		link_clicked();
		if(changeFlag=="B")all_links.click();
	}
})();