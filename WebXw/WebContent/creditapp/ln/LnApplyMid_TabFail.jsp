...
<s:else>ico_arrow_right1</s:else>
							"></div>
							<div class="<s:if test="lnStage.paySts==01">ico_blue</s:if><s:elseif test="lnStage.paySts==02">ico_green</s:elseif><s:else>ico_red</s:else>"><span class="font_desc">放款<br/>
							<s:if test="lnStage.paySts==01">未放款</s:if><s:elseif test="lnStage.paySts==02">已放款</s:elseif><s:else>未放款</s:else></span></div>
							<div class="clear"></div>
						</div>
<form action="#">
			...
			<div class="tab_btn_div">
				<s:if test="flag!='no'">
						...
						<dhcc:button typeclass="back_btn" value="返回列表" action="返回列表"
							onclick="LnApplyMidAction_getTabFail.action"></dhcc:button>
				</s:if>
			</div>
		</td>
	</tr>	 -->
