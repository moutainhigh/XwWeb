...
<script> 
	...
	function getPOP(){ 
		var cardChn = document.getElementsByName("cardChn")[0].value;
		if(cardChn == ''){
		     sAlert("请先选择虚拟账户渠道!");
		     return false;
		}else if(cardChn == 'CL0001'){
		     func_popRadio('POP093','');
		}else if(cardChn == 'CL0002'){
		     func_popRadio('POP094','');
		}
	}
	...
	<s:form method="post" theme="simple" name="cms_form"
		action="ProjAcctActionInsert.action">
		<div class="right_bg">
			<div class="right_w">
				<div class="from_bg">
					<div class="right_v">
						<dhcc:formTag property="formproj0008" mode="query" />
						<div class="from_btn">
							<dhcc:button typeclass="button3" commit="true" value="保存" action="保存"  ></dhcc:button>
							 <s:if test="flag!=1">
							<dhcc:button typeclass="button_form" value="返回" action="返回" onclick="ProjAcctAction_findByPageForVa.action" ></dhcc:button>
						    </s:if>
						     <s:else>
						      <dhcc:button typeclass="button_form" value="返回" action="返回" onclick="ProjAcctAction_findByPageForVa.action" param="projNo=projNo@flag=flag"></dhcc:button>
						     </s:else>
						</div>
					</div>
				</div>
